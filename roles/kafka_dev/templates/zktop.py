...
parser.add_option("", "--servers",
                  dest="servers", default="{% for server in zookeeper_hosts %}{{server.host}}:{{ zookeeper_client_port }}{% if not loop.last %},{% endif %}{% endfor %}",
                  help="comma separated list of host:port (default {% for server in zookeeper_hosts %}{{server.host}}:{{ zookeeper_client_port }}{% if not loop.last %},{% endif %}{% endfor %}")
